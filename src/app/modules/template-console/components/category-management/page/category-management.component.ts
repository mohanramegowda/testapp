...
export class CategoryManagementComponent implements OnInit {
  ...
  constructor(private _database: ChecklistDatabase, public dialog: MatDialog, private router: Router,
    ...
    this.productCategoryService.getAllRootCategories(this.templateId)
      .pipe(
        map(data => {
  ...
  getNodeChildren(node: TreeItemNode) {
    this.productCategoryService.getChildCategories(node.id, this.templateId).subscribe(data => {
  ...
  onPreview() {
    this.router.navigate(['./products-preview', { template: this.templateId }], { relativeTo: this.route });
  }
